...
/*	if (p) \
	...
	{ \
		fprintf(stderr, "libsmacker::smk_malloc(" #p ", %lu) - ERROR: calloc() returned NULL (file: %s, line: %lu)\n\tReason: [%d] %s\n", \
			(unsigned long) (x), __FILENAME__, (unsigned long)__LINE__, errno, strerror(errno)); \
		exit(EXIT_FAILURE); \
	} \
}
