...
///////////////////////////////////////////////////////////////////////////////
PER_DEFINE_PERSISTENT( RenIVertexData );
...
void RenIVertexData::createIntensityMap()
{
	...
	if (!intensityMap())
	{
		...
		intensityMap_ = _NEW(RenIVertexIntensities(std::min(maxInitialSize, size())));
	}
...
void RenIVertexData::createMaterialMap()
{
	...
	{
		const size_t maxInitialSize = 10;
		materialMap_ = _NEW(RenIVertexMaterials(std::min(maxInitialSize, size())));
	}
...
void RenIVertexData::addVertex(const MexPoint3d& pt, const MexVec3& normal, const MexPoint2d& uv)
{
	...
	static const uint black = packColourChecked(0,0,0);
...
ostream& operator <<( ostream& o, const RenIVertexData& data )
{
    const RenIVertexData::Base& base = data;
...
ostream& operator<<(ostream& os, const RenIVertex& vtx)
{
