...
MachLogAdminDeconstructOperation::MachLogAdminDeconstructOperation( MachLogAdministrator * pActor, MachLogConstruction* pConstruction_ )
	     ...
	     i!=pActor_->squadron()->machines().end(); ++i )
    {
			  ...
			  (*i)->strategy().currentOperationType() == MachLogOperation::FOLLOW_OPERATION ) )
		{
			...
			if( (*i)->objectType() == MachLog::CONSTRUCTOR /* and  allConstructorsIdle*/ )
			{
				...
				pConstructor->newOperation( _NEW( MachLogDeconstructOperation( pConstructor, pConstruction_ ) ) );
...
//virtual
bool MachLogAdminDeconstructOperation::beNotified( W4dSubject* pSubject,
