...
debrisType_(debrisType)
{
	...
	const MexAlignedBox2d& localDebrisBoundary = boundary;
...
*/
    ...
    debrisUnits_.reserve( nDebrisUnits );

	for (int debrisLoop = 0; debrisLoop < nDebrisUnits; ++debrisLoop)
	{
		double xVal = 0.0; //MachPhysRandom::randomDouble(-1 * halfXSize, halfXSize);
    	double yVal = 0.0; //MachPhysRandom::randomDouble(-1 * halfYSize, halfYSize);
