...
/*
    MachGuiTreacheryCommand

    Implement interaction for the Attack command
*/

#ifndef _MACHGUI_CMDTRECH_HPP
#define _MACHGUI_CMDTRECH_HPP
...
class MachGuiTreacheryCommand : public MachGuiCommand
{
public:
    //ctor.
    MachGuiTreacheryCommand( MachInGameScreen* pInGameScreen );

    //dtor
    virtual ~MachGuiTreacheryCommand();
    ...
    friend ostream& operator <<( ostream& o, const MachGuiTreacheryCommand& t );
...
private:
    ...
    bool applyTreacheryObject( MachActor* pActor, string* pReason );

    //Data members
    Action action_; //What to do
    MachActor* pDirectObject_; //The target of the Treachery
