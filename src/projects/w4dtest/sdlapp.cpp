...
PER_DEFINE_PERSISTENT( SDLAppPersistence );
...
bool SDLApp::readModels(const SysPathName& pathname, MexTransform3d* cameraXform)
{
...
void SDLApp::readEnvironment(const SysPathName& planetName)
{
	PRE(device_);
	PRE(light_);
...
void SDLApp::pickDisplayMode(bool windowMode, int width, int height)
{
	...
	if (!windowMode)
	{
        ...
        display_->useMode(width, height, 0);

	std::cout << *display_ << std::endl;
}

bool SDLApp::readBackground
(
...
// virtual
//bool SDLApp::activate(WORD /*wParam*/)
//{
//	return true;
//}
...
SDLAppPersistence::SDLAppPersistence( SDLApp* pApp )
: pSDLApp_( pApp )
{
}

SDLAppPersistence::SDLAppPersistence( PerConstructor )
{
    ASSERT_FAIL( "This should never be called" );
}

void perWrite( PerOstream& ostr, const SDLAppPersistence& per )
{
    ostr << per.pSDLApp_->root_;
    ostr << per.pSDLApp_->models_;
}

void perRead( PerIstream& istr, SDLAppPersistence& per )
{
    istr >> per.pSDLApp_->root_;
    istr >> per.pSDLApp_->models_;
}
